...
#   ./install-ai-rules.sh --download [target-directory]
#
# Modes:
#   (default)    Generates files inline from the embedded content below.
#   --download   Downloads pre-generated files from the guide repository.
#
...
REPO_RAW_URL="https://raw.githubusercontent.com/rios0rios0/guide/main"
DOWNLOAD_MODE=false
...
# ---------------------------------------------------------------------------
# Download mode: fetch pre-generated files from the repository
# ---------------------------------------------------------------------------
if [[ "$DOWNLOAD_MODE" == true ]]; then
  echo "  (downloading from $REPO_RAW_URL)"
  curl -fsSL "$REPO_RAW_URL/CLAUDE.md" -o "$TARGET_DIR/CLAUDE.md"
  echo "  -> downloaded CLAUDE.md"

  curl -fsSL "$REPO_RAW_URL/.cursorrules" -o "$TARGET_DIR/.cursorrules"
  echo "  -> downloaded .cursorrules"

  curl -fsSL "$REPO_RAW_URL/AGENTS.md" -o "$TARGET_DIR/AGENTS.md"
  echo "  -> downloaded AGENTS.md"

  echo ""
  echo "Done. All AI rule files have been downloaded with the full development guide conventions."
...
# ---------------------------------------------------------------------------
# Inline mode: generate files from embedded content
# ---------------------------------------------------------------------------
