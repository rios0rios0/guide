...
When no argument is given, it defaults to the current working directory (`$PWD`). If the user provides a specific subdirectory (e.g. `dev.azure.com/ContosoSecurity`), pass it as argument:

```bash
source ~/.scripts/linux-engineering-git-sync-repos.sh && git-sync-repos "$HOME/Development/dev.azure.com/ContosoSecurity"
```
