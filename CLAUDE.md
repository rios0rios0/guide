...
Markdown files (source of truth)
  ...
  ├─→ generate-ai-rules tool ─→ .ai/ directory (Claude/Cursor/Codex rule files)
  └─→ install-rules.sh ─→ distributes .ai/ files to projects or ~/.claude/, ~/.cursor/, etc.
```
...
The `.ai/` directory is auto-generated by the `generate-ai-rules` tool and committed to the repo. It contains:
- `claude/rules/` — 14 rule files (`.md`)
- `claude/commands/` — 5 slash commands
- `claude/agents/` — 1 agent (chezmoi)
- `cursor/rules/` — 14 rule files (`.mdc`)
- `cursor/skills/` — 4 skills
- `codex/` — `AGENTS.md` and `rules/default.rules`
...
| `generate-ai-rules.yaml` | Push to `main` (doc paths), manual | Regenerates `.ai/` from docs |
| `sync-docs.yaml` | PR with `.md` changes | Validates TOC sync |

## Install Script

`install-rules.sh` downloads generated rule files from GitHub and installs them globally (`~/.claude/`, `~/.cursor/`, etc.) or into a specific project directory. Supports `--force` to skip overwrite prompts.
